...
def create_exceltree(outputdir):
  ...
  for dirpath, subdirnames, files in os.walk(outputdir):
    ...
    for name in sorted(files):
      ...
      if compilername not in exceltree.compilersarray:
        ...
        exceltree.threadsnumarray.append([])
        exceltree.threadsnumstartingcol.append([])
      ...
      threadsnum = name.rsplit('-', 1)[1].split('.', 1)[0]

      print(threadsnum)
      print(len(exceltree.threadsnumarray[compilerindex]))

      ## If the optimization name does not exist in the optimization list of the current compiler, append it ##
      if not any(threadsnum in x for x in exceltree.threadsnumarray[compilerindex]):
        exceltree.threadsnumarray[compilerindex].append(threadsnum)

      print(exceltree.threadsnumarray[compilerindex])
      print(len(exceltree.threadsnumarray[compilerindex]))
  ...
  ## compilerstartingcol[i] = compilerstartingcol[i] + len(optimizationarray[i]) * totaltestsnum ##
  for i in range(0, len(exceltree.compilersarray)):
    ...
    if (i == 0):
      ...
      exceltree.compilerstartingcol.append(exceltree.compilerstartingcol[i - 1] + len(exceltree.threadsnumarray[i - 1]))

    ## Loop all optimizations of a compiler and calculate their starting column ##
    ## startingcol[i][j] = startingcol[i][j] + totaltestsnum                   ##
    for j in range(0, len(exceltree.threadsnumarray[i])):

      print("j = " + str(j))

      ## The first optimization column starts at the same column as the compiler ##
      if (j == 0):
        exceltree.threadsnumstartingcol[i].append(exceltree.compilerstartingcol[i])

      else:
        exceltree.threadsnumstartingcol[i].append(exceltree.threadsnumstartingcol[i][j - 1] + totaltestsnum)
...
def dump_exceltree_info(exceltree):
  ...
  for i in range(0, len(exceltree.compilersarray)):
    ...
    print('\t' + "Threads number list: { " + str(exceltree.threadsnumarray[i]) + "}")
...
def build_excel_spreadsheet(exceltree, builddir, outputdir):
  ...
  for dirpath, subdirnames, files in os.walk(outputdir):
    ...
    for name in sorted(files):
      ...
      max_index = dataframe[["Clustering time"]].idxmax()
      min_index = dataframe[["Clustering time"]].idxmin()
      ...
      # threadsnum = name.split('-', 1)[1].split('.', 1)[0]
      threadsnum = name.rsplit('-', 1)[1].split('.', 1)[0]

      threadsnumindex = exceltree.threadsnumarray[compilerindex].index(threadsnum)

      # startingcol = exceltree.compilerstartingcol[compilerindex] + (threadsnumindex) + (int(testnum)) - 1
      # startingcol = exceltree.compilerstartingcol[compilerindex] + (len(exceltree.threadsnumarray[compilerindex]) * int(testnum)) + threadsnumindex
      startingcol = exceltree.compilerstartingcol[compilerindex] + (totaltestsnum * threadsnumindex) + int(testnum) - 1

      print(compilername)
      print(str(compilerindex))
      print(exceltree.compilerstartingcol[compilerindex])
      print(testnum)
      print('Threadsnumindex = ' + str(threadsnumindex))
  ...
  for i in range(0, len(exceltree.compilersarray)):
    ...
    print("totaltestsnum = " + str(totaltestsnum) + " threadsnumarray[i] = " + str(len(exceltree.threadsnumarray[i])))
    compilercellsize = len(exceltree.threadsnumarray[i]) * totaltestsnum
    ...
    for j in range(0, len(exceltree.threadsnumarray[i])):
      ...
      threadsnumcellsize = totaltestsnum

      sheet1.cell(row=(datastartrow + 1), column=(exceltree.threadsnumstartingcol[i][j])).value = exceltree.threadsnumarray[i][j]
      sheet1.merge_cells(start_row=(datastartrow + 1), start_column=(exceltree.threadsnumstartingcol[i][j]), end_row=(datastartrow + 1), end_column=(exceltree.threadsnumstartingcol[i][j] + totaltestsnum - 1))

      testcellstartingcol = exceltree.compilerstartingcol[i] + threadsnumcellsize * j
      ...
      for k in range(0, len(testarray.testname)):
        ...
        testcellstartingcol = testcellstartingcol + 1
  ...
  ## Calculate Average Clustering Time and PSNR, calculate ##
  ...
  for dirpath, subdirnames, files in os.walk(outputdir):
    ...
    for name in files:
      ...
      ## Get the optimization flags ##
      ## Split the line on the first '-' encountered and keep the right part ##
      threadsnum = name.rsplit('-', 1)[1].split('.')[0]
      ...
      # startingcol = exceltree.compilerstartingcol[compilerindex] + (threadsnumcellsize * threadsnumindex) + testnum
      startingcol = exceltree.compilerstartingcol[compilerindex] + (len(exceltree.threadsnumarray[compilerindex]) * int(testnum)) + threadsnumindex

      startingcell = xlref(datastartrow + 4, startingcol, False)
      endingcell = xlref(datastartrow + 4 + testiterations - 1, startingcol, False)

      ## Calculate the Average clustering time ##
      average_clustering_time_cell = xlref(datastartrow + 4 + testiterations, startingcol, False)

      print('= AVERAGE(' + startingcell + ':' + endingcell + ')')

      sheet1[average_clustering_time_cell] = '= AVERAGE(' + startingcell + ':' + endingcell + ')'
      ...
      ## Calculate the Standard Deviation of clustering time ##
      stdev_clustering_time_cell = xlref(datastartrow + 5 + testiterations, startingcol, False)

      print('= STDEV.P(' + startingcell + ':' + endingcell + ')')

      sheet1[stdev_clustering_time_cell] = '= STDEV.P(' + startingcell + ':' + endingcell + ')'
