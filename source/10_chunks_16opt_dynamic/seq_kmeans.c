...
double seq_kmeans(float **objects,      /* in: [numObjs][numCoords] */
               ...
               float **clusters)     /* out: [numClusters][numCoords] */
{
  ...
  do
    {
      ...
      #pragma omp parallel shared(objects, clusters, membership, newClusters, newClusterSize)
        {
          #pragma omp for reduction(+:delta) private(j, index) schedule(dynamic, 1105)
          for (i = 0; i < numObjs; i++)
            {
          ...
          #pragma omp for simd private(j), schedule(dynamic, 124)
  ...
  clustering_time = wtime() - clustering_time;
